...
library(simpleaffy)
celfiles <- read.affy(covdesc="phenodataUCN.txt", path="data")
...
arrayQualityMetrics(celfiles, outdir = "QAraw_UCN", force = FALSE, do.logtransform = TRUE, intgroup = "dis", reporttitle = paste("Quality metrics report for", "QCraw_CDN"))

arrayQualityMetrics(celfiles.rma, outdir = "QARMA_UCN", force = FALSE, do.logtransform = TRUE, intgroup = "dis", reporttitle = paste("Quality metrics report for", "QCRMA_CDN"))
...
results <- cbind(top, gene.symbols)
results <- data.frame(results,eset2[row.names(top),])
write.table(results,file="UC4183_1.5-01.xls",sep='\t')
